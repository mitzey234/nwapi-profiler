...
public static class PlayerListUtils
{
    static PlayerListUtils()
    {
        ...
        {
            AllHubs.Add(hub);
            InstanceModeChanged(hub, hub.authManager._targetInstanceMode);
        }

        ReferenceHub.OnPlayerAdded += PlayerAdded;
        ReferenceHub.OnPlayerRemoved += PlayerRemoved;
        PlayerAuthenticationManager.OnInstanceModeChanged += InstanceModeChanged;
    }
    ...
    {
        if (hub.authManager._targetInstanceMode != ClientInstanceMode.ReadyClient)
            return false;
